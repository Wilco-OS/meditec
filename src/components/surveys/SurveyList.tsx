            ...
            <Card key={survey.id} className="flex flex-col">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {isAdmin && (
                      <>
                        <Link 
                          href={survey.id && survey.id !== 'undefined' ? `/admin/surveys/${survey.id}/edit` : '#'}
                          onClick={(e) => {
                            if (!survey.id || survey.id === 'undefined') {
                              e.preventDefault();
                              console.error('Ungültige Umfrage-ID verhindert Navigation:', survey.id);
                              toast({
                                title: 'Fehler',
                                description: 'Diese Umfrage kann nicht bearbeitet werden (ungültige ID).',
                                variant: 'destructive',
                              });
                            } else {
                              console.log('Navigation zur Bearbeitungsseite mit ID:', survey.id);
                            }
                          }}>
                          {/* Debug-Information */}
              ...
              <CardFooter className="pt-4">
                <Button asChild className="w-full">
                  <Link 
                    href={survey.id && survey.id !== 'undefined' 
                      ? (isAdmin 
                          ? `/admin/surveys/${survey.id}/edit` 
                          : `/company/surveys/${survey.id}`) 
                      : '#'} 
                    onClick={(e) => {
                      // Validiere die ID vor der Navigation
                      if (!survey.id || survey.id === 'undefined') {
                        e.preventDefault();
                        console.error('Ungültige Umfrage-ID verhindert Navigation:', survey.id);
                        toast({
                          title: 'Fehler',
                          description: 'Diese Umfrage kann nicht angezeigt werden (ungültige ID).',
                          variant: 'destructive',
                        });
                      } else {
                        console.log('Navigation zur Umfragedetailseite mit ID:', survey.id);
                      }
                    }}>
